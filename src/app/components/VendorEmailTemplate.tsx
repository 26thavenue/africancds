export const vendorTicketPendingEmail = {
  subject: "Vendor Ticket Order Received – ACDS Summit 2025",
  html: `
    <h2 style="font-size: 24px; font-weight: bold; color: #1a2e4f; text-align: center;">
      Welcome to the African Chiefs of Defence Summit
    </h2>
    <p>
      Thank you for choosing to participate as a vendor at the African Chiefs of Defence Summit 2025.
    </p>
    <p>
      We have received your request for a <strong>Vendor Ticket</strong>. The vendor ticket fee is <strong>$3,000</strong>.
    </p>
    <p>
      You will receive a confirmation email once your payment is verified. Kindly ensure your payment is completed to secure your spot.
    </p>
    <p>
      For any questions or assistance, feel free to reach out to our support team.
    </p>
    <p>Best regards,<br/>ACDS Summit Team</p>
  `,
};
